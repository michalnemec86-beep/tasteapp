-- Keep the private integrity smoke test aligned with the "brewed for" model.
-- A beer-level brewery may legitimately be either the current producer or
-- the brewery the current version is brewed for. Brand ownership may likewise
-- live on the commissioned-for brewery rather than the physical producer.
...
current_payload_mismatch as (
  ...
  join public.beer_versions bv
    on bv.beer_id = be.id
   and bv.is_current
  where be.plato is distinct from bv.plato
...
current_brewery_reference_mismatch as (
  select be.id
  from public.beers be
  join public.beer_versions bv
    on bv.beer_id = be.id
  ...
  where be.brewery_id is distinct from bv.brewery_id
    and (
      bv.brewed_for_brewery_id is null
      or be.brewery_id is distinct from bv.brewed_for_brewery_id
    )
),
...
select jsonb_build_object(
      ...
      where be.brand_id is not null
        ...
        and not exists (
          ...
          join public.brewery_brands bb
            on bb.brewery_id = bv.brewed_for_brewery_id
           and bb.brand_id = be.brand_id
          where bv.beer_id = be.id
            and bv.is_current
            and bv.brewed_for_brewery_id is not null
        )
    ),
  ...
  'current_brewery_reference_mismatches',
    (select count(*) from current_brewery_reference_mismatch),
